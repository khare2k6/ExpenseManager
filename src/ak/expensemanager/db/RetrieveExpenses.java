...
public class RetrieveExpenses implements IRetrieveExpenses{
	...
	final String TAG =IDebugTag.ankitTag +RetrieveExpenses.class.getSimpleName();
	...
	public ArrayList<MonthlyExpense> getAllMonthExpenses() {
		...
		for(IDebugTag.Months month : IDebugTag.Months.values()){
			...
			Log.d(TAG,"exp for month:"+exp.getMonth()+" = "+exp.getTotalExpense());
			list.add(exp);
		}
		return list;
	}
	...
	public int getMonthlyExpense(String month) {
		...
		if(cursor.getCount() >0){
			cursor.moveToFirst();
			do{
				long dateInMs = cursor.getLong(cursor.getColumnIndex(DbHelper.C_DATE));
				...
				if(month.equalsIgnoreCase(thisRecordsMonth)){
					int thisRecordExpense = cursor.getInt(cursor.getColumnIndex(DbHelper.C_AMOUNT));
	...
	public class DbHelper extends SQLiteOpenHelper{
		...
		public  final String TAG = IDebugTag.ankitTag +DbHelper.class.getSimpleName();
		...
		public void onCreate(SQLiteDatabase db) {
			...
			String sql ="create table "+TABLE_NAME+" ( " +C_ID+ " integer primary key autoincrement, "	
					+C_DATE+" int,"+C_AMOUNT+", int, "+C_CATEGORY +" text, "+C_NOTES+" text)";
			db.execSQL(sql);
	...
	public Cursor getMonthlyExp(int month){
		...
		MatrixCursor matCursor = new MatrixCursor(new String[] {DbHelper.C_ID,DbHelper.C_DATE, DbHelper.C_AMOUNT});
		int dailExp =0;
		int totalEntries = 1;
		int previousDate =0;
		if(cursor.getCount() > 0){
		...
		long startDateLong =  cursor.getLong(cursor.getColumnIndex(DbHelper.C_DATE));
		int startDateInt = Integer.parseInt((new SimpleDateFormat("dd")).format(new Date(startDateLong))); 
		previousDate = startDateInt;
		previousDateLong = startDateLong;
		// sum up expense of single days
		do{
			startDateLong = cursor.getLong(cursor.getColumnIndex(DbHelper.C_DATE));
			startDateInt = Integer.parseInt((new SimpleDateFormat("dd")).format(new Date(startDateLong))); 
			Log.d(TAG,"sDate ="+startDateInt );
			
			if(startDateInt == previousDate){
				dailExp += cursor.getInt(cursor.getColumnIndex(DbHelper.C_AMOUNT));
			}else{
				//enter into cursor
				matCursor.addRow(new Object  [] {totalEntries++,getDate(previousDateLong),dailExp});
				previousDate = startDateInt;
				previousDateLong = startDateLong;
		...
		matCursor.addRow(new Object  [] {totalEntries++,getDate(startDateLong),dailExp});
		}
	...
	public Cursor geMontlyExpenseCursor(int month) {
		...
		Calendar startDateCalender = new GregorianCalendar(cal.get(Calendar.YEAR), month, 1);
		Calendar endDateCalender = new GregorianCalendar((cal.get(Calendar.YEAR)),month,
		...
		String selection = DbHelper.C_DATE +" BETWEEN "+startDate + " AND "+endDate;
		Cursor cursor = db.query(DbHelper.TABLE_NAME, null, selection, null, null, null, null);
		Log.d(TAG,"geMontlyExpenseCursor reutning cursor.getcoutn="+cursor.getCount());
		return cursor;
	}
	...
	public Cursor getExpenseBetweenDates(long startDate,long endDate) {
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		Calendar cal = Calendar.getInstance();
		Log.d(TAG,"start date= "+getDate(startDate)+"end date = "+getDate(endDate));
		String selection = DbHelper.C_DATE +" BETWEEN "+startDate + " AND "+endDate;
		Cursor cursor = db.query(DbHelper.TABLE_NAME, null, selection, null, null, null, null);
